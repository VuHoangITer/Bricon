...
# ==================== BULK IMPORT QUESTIONS (BONUS) ====================

@quiz_admin_bp.route('/questions/import/<int:quiz_id>', methods=['GET', 'POST'])
@permission_required('manage_quiz')
def import_questions(quiz_id):
    """
    Import hàng loạt câu hỏi từ file JSON/CSV
    Format JSON:
    [
      {
        "question": "...",
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for item in data:
                ...
                for idx, answer_text in enumerate(item['answers']):
                    answer = Answer(
                        question_id=question.id,
            ...
            flash(f'✅ Đã import {len(data)} câu hỏi thành công!', 'success')
            return redirect(url_for('quiz_admin.edit_questions', quiz_id=quiz_id))
